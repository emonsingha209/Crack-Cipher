...
export default function encode() {
  ...
  function convert(text, shift) {
    ...
    for (let i = 0; i < text.length; i++) {
      ...
      if(asciiCode >= 65 && asciiCode <= 90) {
        shiftedValue = ((parseInt(asciiCode) - 65 - parseInt(shift) + 26) % 26) + 65;
      }
      else if(asciiCode >= 48 && asciiCode <= 57) {
        shiftedValue = ((parseInt(asciiCode) - 48 - parseInt(shift) + 10) % 10) + 48;
      }
      else {
  ...
  const handleSubmit = (e) => {
    ...
    let ok;
    for(let i=1; i<=25; i++) {
      ok = convert(ciphertext.toUpperCase(), i);
      result.push(ok + ' - ' + i + ' | ');
    }
    //ok = convert(ciphertext.toUpperCase(), shift);
    //result.push(ok);
    setPlaintext(result);
  };
  return (
    ...
    <main className='flex items-center justify-center'>
      <div className='w-3/5 h-fit p-4 flex flex-col items-center justify-center rounded text-2xl'>
        <form className='flex flex-col w-3/5 gap-4' onSubmit={handleSubmit}>
          ...
          <label className='flex flex-col gap-2'>
            Shift: 
            (If shift key unknown, leave as shift key 0. Check one or two words for shift key discovery.)
            <input
              type='number'
              value={shift}
